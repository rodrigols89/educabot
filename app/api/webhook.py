...
from app.services.gestor_service import find_gestor
from app.services.notification_service import build_supplier_message
from app.services.pedido_service import save_request
...
async def evolution_webhook(
    ...
    if not is_command:
        print("COMANDO INVÁLIDO OU IGNORADO")
        print("========================================\n")
    ...
    try:
        ...
        if gestor is None:
            print("RESPONSÁVEL PELO PEDIDO NÃO ENCONTRADO")
            print(f"Telefone: {phone}")
            print("========================================\n")

            return {"status": "received"}

        print("RESPONSÁVEL PELO PEDIDO ENCONTRADO")
        print(f"Nome: {gestor.nome}")
        print(f"Telefone: {gestor.telefone}")
        print(f"Instituição: {gestor.instituicao}")

        # =========================
        # SALVAR PEDIDO
        # =========================
        pedido, tipo = save_request(
            db=db,
            gestor=gestor,
            command=text,
        )
        ...
        if pedido is None:
            ...
            message = (
                ...
                "Aguarde o atendimento do seu pedido atual."
            )
            ...
            print("\nPEDIDO BLOQUEADO (DUPLICADO)")
            print("========================================")
            print(f"Gestor: {gestor.nome}")
            print(f"Tipo: {tipo}")
            print("========================================\n")
        ...
        print(f"Gestor ID: {pedido.gestor_id}")
        print(f"Tipo: {pedido.tipo.value}")
        ...
        print("========================================")
        print(f"Fornecedor: {supplier_phone}")
        ...
        if sent:
            ...
            confirmation_sent = send_text_message(
                phone=phone,
            ...
            print("Falha ao enviar mensagem para fornecedor")
